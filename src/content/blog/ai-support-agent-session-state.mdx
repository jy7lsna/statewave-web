...
Sessions are also the natural unit further down the stack. In production agent traces analysed for the [SMetric scheduling paper](https://arxiv.org/abs/2607.08565), KV cache reuse exceeded 80% of request tokens, against 54% to 62% for human chat. Agent workloads cluster inside sessions. Ranking that ignores the boundary fights the shape of the traffic.
...
Session and subject scoping are not tenant scoping. An agent can be correctly authenticated, correctly authorised, and still receive another customer's facts in a bundle because a surface downstream of the boundary was never scoped. We wrote up [which surfaces actually need a tenant boundary](/blog/multi-tenant-isolation-in-ai-memory), including the backfill step that silently erases memory if you skip it. Read it before your second tenant, not after.
...
**Start with the state layer.** Adding memory to a support agent without session state gives you an agent that remembers everything and prioritises nothing. The four fields that fix it, a subject ID, a session ID, a status, and a summary, cost an afternoon. [Boot it locally in two minutes](/developers) or read the ranking code yourself in the [core runtime](https://github.com/smaramwbc/statewave), which is Apache-2.0 and self-hosted end-to-end.
